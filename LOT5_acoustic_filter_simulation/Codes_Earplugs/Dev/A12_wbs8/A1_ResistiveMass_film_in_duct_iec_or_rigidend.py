...
def plot_iec_il_cases(case_results: dict[str, dict[str, np.ndarray]]) -> None:
    ...
    ax.set_ylabel("IL [dB]")
    ax.set_title("IEC711 insertion loss for all film cases")
    ax.grid(True, which="both", alpha=0.3)
    ax.legend(loc="best")
    plt.show()
...
if __name__ == "__main__":
    ...
    film_cases = {
        "mesh_60": {
            "film_resistance": 1.0e6,   # Rf [Pa.s/m^3]
            "film_mass": 1.0e-2,        # Mf [Pa.s^2/m^3]
            "fem_file": HERE / "fem_rslt" / "rslt_fem_rf1e6_mf1em2.txt",
        },
        "mesh_300": {
            "film_resistance": 1.0e8,   # Rf [Pa.s/m^3]
            "film_mass": 1.0e-2,        # Mf [Pa.s^2/m^3]
            "fem_file": HERE / "fem_rslt" / "rslt_fem_rf1e8_mf1em2.txt",
        },
        "mesh_500": {
            "film_resistance": 1.0e10,  # Rf [Pa.s/m^3]
            "film_mass": 1.0e-2,        # Mf [Pa.s^2/m^3]
            "fem_file": HERE / "fem_rslt" / "rslt_fem_rf1e10_mf1em2.txt",
        },
    }
    active_case = "mesh_500"
