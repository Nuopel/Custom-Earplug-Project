...
if __name__ == "__main__":
    ...
    film_cases = {
        "zn_1p32e5": {
            "film_specific_impedance": 1.32e5,  # FEM interior impedance boundary value Zn [Pa.s/m]
            "fem_file": HERE / "fem_rslt" / "rslt_fem_zn_1p32e5.txt",
        },
        "zn_1p28e6": {
            "film_specific_impedance": 1.28e6,  # FEM interior impedance boundary value Zn [Pa.s/m]
            "fem_file": HERE / "fem_rslt" / "rslt_fem_zn_1p28e6.txt",
        },
    }
    active_case = "zn_1p28e6"
    case_cfg = film_cases[active_case]
    film_specific_impedance = case_cfg["film_specific_impedance"]
    FEM_FILE = case_cfg["fem_file"]

    s_in = r_inlet**2*np.pi
    s_out = r_outlet**2*np.pi
    film_resistance = film_specific_impedance / s_in  # Acoustic series resistance [Pa.s/m^3]
    ...
    halfduct = CylindricalDuct(radius=r_duct, length=length_duct/2, c0=C0, rho0=RHO0)
    film = GenericFilmSeriesImpedance(resistance=film_resistance)
