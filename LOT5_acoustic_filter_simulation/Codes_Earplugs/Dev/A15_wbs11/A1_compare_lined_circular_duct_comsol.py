...
from A0_minimal_lined_circular_duct import (
    LinedCylindricalDuct,
)
from toolkitsd.acoustmm import CylindricalDuct, FrozenMatrixElement
...
def plot_tl_comparison(
    ...
    plt.semilogx(freqs_hz, tl_tmm_db, linewidth=2.0, label="TMM lined circular duct")
    ...
    plt.title("Circular lined duct: FEM vs TMM")
...
def plot_matrix_comparison(
    ...
    *,
    ...
    fig.suptitle("Circular lined duct: matrix comparison (FEM vs TMM)")
...
if __name__ == "__main__":
    ...
    length_duct = 0.05
    radius_cyl = 0.01
    rigid_first_mode_cutoff_hz = 1.84 * C0 / (2.0 * np.pi * radius_cyl)

    fem_file_zs = HERE / "fem_rslt" / "A0_circularduct_with_lining_zs_jca.txt"
    fem_file_porous = HERE / "fem_rslt" / "A0_circularduct_with_lining.txt"
    area = np.pi * radius_cyl**2
    ...
    inlet = CylindricalDuct(radius=radius_cyl, length=length_inlet, c0=C0, rho0=RHO0)
    outlet = CylindricalDuct(radius=radius_cyl, length=length_outlet, c0=C0, rho0=RHO0)
    ...
    print(f"Rigid circular duct first cutoff ≈ {rigid_first_mode_cutoff_hz:.1f} Hz")
    print(f"FEM frequency range: {freqs_fem[0]:.1f} Hz -> {freqs_fem[-1]:.1f} Hz")
    tmm_lined_duct = LinedCylindricalDuct(
        radius=radius_cyl,
        length=length_duct,
    ...
    # plot_kz_diagnostics(freqs_fem, kz_circular, rigid_first_mode_cutoff_hz)
    # plt.show()
