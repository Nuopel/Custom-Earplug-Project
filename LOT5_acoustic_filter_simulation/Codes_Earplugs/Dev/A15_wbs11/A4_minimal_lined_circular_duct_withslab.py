...
from toolkitsd.acoustmm import AcousticElement, CylindricalDuct, ElasticSlab
...
def rigid_backed_surface_impedance_from_equivalent_section(
    ...
    *,
    ...
    """Build the wall surface impedance seen from air from an equivalent slab section.

    ``zc_eq`` is an acoustic characteristic impedance in [Pa.s/m^3]. The lined-duct
    model expects a wall surface impedance in [Pa.s/m], so we first convert back to
    specific impedance using the slab cross-sectional area.
    """
...
def plot_equivalent_params(freqs_hz: np.ndarray, k_eq: np.ndarray, zc_eq: np.ndarray, zs_wall: np.ndarray) -> None:
    fig, axes = plt.subplots(3, 1, figsize=(10, 10), sharex=True, constrained_layout=True)

    axes[0].semilogx(freqs_hz, np.abs(k_eq), linewidth=2.0, label=r"$|k_{eq}|$")
    axes[0].semilogx(freqs_hz, np.angle(k_eq), "--", linewidth=2.0, label=r"Phase($k_{eq}$)")
    axes[0].set_ylabel(r"$k_{eq}$")
    axes[0].grid(True, which="both", alpha=0.3)
    axes[0].legend(loc="best")

    axes[1].semilogx(freqs_hz, np.abs(zc_eq), linewidth=2.0, label=r"$|Z_{c,eq}|$")
    axes[1].semilogx(freqs_hz, np.angle(zc_eq), "--", linewidth=2.0, label=r"Phase($Z_{c,eq}$)")
    axes[1].set_ylabel(r"$Z_{c,eq}$")
    axes[1].grid(True, which="both", alpha=0.3)
    axes[1].legend(loc="best")

    axes[2].semilogx(freqs_hz, np.abs(zs_wall), linewidth=2.0, label=r"$|Z_s|$")
    axes[2].semilogx(freqs_hz, np.angle(zs_wall), "--", linewidth=2.0, label=r"Phase($Z_s$)")
    axes[2].set_xlabel("Frequency [Hz]")
    axes[2].set_ylabel(r"$Z_s$")
...
if __name__ == "__main__":
    ...
    matrix_slab = slab.matrix(omega)

    k_eq, zc_eq = extract_equivalent_section_from_matrix(matrix_slab, omega, l_slab)
    s_slab = np.pi * r_slab**2
    zs_wall = rigid_backed_surface_impedance_from_equivalent_section(
        k_eq,
        zc_eq,
        thickness=l_slab,
        area=s_slab,
    ...
    plot_equivalent_params(freqs_hz, k_eq, zc_eq, zs_wall)
    plt.show()
