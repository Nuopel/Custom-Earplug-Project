"""Two-load FEM matrix identification for the WBS5 diff-radius verification case."""
...
from toolkitsd.acoustmm import CylindricalDuct, WaveguideElement


OPEN_PATH = VERIF_DIR / "diff_radius_silicone_air_open.txt"
RIGID_PATH = VERIF_DIR / "diff_radius_silicone_air_rigidend.txt"
...
POINT_TOTAL_LEFT = 9
POINT_TOTAL_RIGHT = 11
POINT_SLAB_LEFT = 9
POINT_SLAB_RIGHT = 10

UPSTREAM_RADIUS_M = 14.5e-3
RADIUS_M = 3.75e-3
...
UPSTREAM_AREA_M2 = np.pi * UPSTREAM_RADIUS_M**2
...
def state_at(case: FemCase, point_id: int) -> np.ndarray:
    ...
    area_m2 = AREA_BY_POINT[point_id]
...
def print_summary(
    ...
    print("=== WBS5 FEM TWO-LOAD MATRIX IDENTIFICATION (DIFF RADIUS) ===")
...
def plot_results(
    ...
    fig_total.suptitle("Identified Total Matrix (Diff Radius)")
    plt.show()
    ...
    fig_slab.suptitle("Slab Matrix: Direct vs Decascaded (Diff Radius)")
    plt.show()
    ...
    fig_pressures.suptitle("End Pressure and Rigid-End IL (Diff Radius)")
    plt.show()
...
if __name__ == "__main__":
    ...
    np.savez(
        HERE / "diff_radius_identified_matrices_from_fem.npz",
