...
from toolkitsd.acoustmm import (
    ...
    IEC711Coupler,
    JCALayer,
    ViscothermalDuct,RadiationImpedance
)
...
if __name__ == "__main__":
    ...
    porous_case = {
        "material_preset": "melamine_cttm",
        "thickness": 15e-3,
        "fem_file": HERE / "fem_rslt" / "rslt_fem_jca_layer.txt",
    }
    FEM_FILE = porous_case["fem_file"]
    ...
    duct = CylindricalDuct(radius=r_jca, length=porous_case["thickness"], c0=C0, rho0=RHO0)
    mat = PorousMaterial.get_material_preset(porous_case["material_preset"], rho0=RHO0, c0=C0)
    porous_layer = JCALayer(
        ...
        length=porous_case["thickness"],
        area=s_jca,
        rho0=RHO0,
        c0=C0,
        name=f"JCA layer ({porous_case['material_preset']})",
    )

    print(mat)

    system_filter =   inlet  + porous_layer  + outlet
