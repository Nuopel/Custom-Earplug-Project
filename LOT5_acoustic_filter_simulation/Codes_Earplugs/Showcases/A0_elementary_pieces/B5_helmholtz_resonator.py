...
def plot_pressure_and_il_results(
    ...
    *,
    ...
    axes[0].semilogx(
        freqs_hz,
        20.0 * np.log10(np.maximum(np.abs(p_end_iec711_hr), np.finfo(float).tiny)),
        lw=2.0,
        label="Duct + HR shunt, IEC711 load",
    )
...
if __name__ == "__main__":
    ...
    freqs = np.logspace(np.log10(50.0), np.log10(4000.0), 300)
    ...
    halfduct = CylindricalDuct(radius=r_duct, length=length_duct / 2.0, c0=C0, rho0=RHO0)
    straight_duct = CylindricalDuct(radius=r_inlet, length=length_duct, c0=C0, rho0=RHO0)

    resonator = HelmholtzResonator(
        radius_neck=0.5e-3,
        length_neck=1.5e-3,
        radius_cavity=10.0e-3,
        length_cavity=34.0e-3,
        radius_waveguide=r_duct,
        outside_flanged=False,
        c0=C0,
        rho0=RHO0,
        use_losses=True,
        include_end_correction=True,
    )
    resonator_shunt = HelmholtzResonatorShunt(resonator)

    system_air_only = inlet + straight_duct + outlet
    system_filter = inlet + halfduct + resonator_shunt + halfduct + outlet
    ...
    f0_lumped = resonator.helmholtz_frequency_lumped_approximate()
    f0_num = resonator.helmholtz_frequency_numerical()
    print(f"Lumped Helmholtz estimate: {f0_lumped:.1f} Hz")
    print(f"Numerical Helmholtz estimate: {f0_num:.1f} Hz")
    ...
    plot_pressure_and_il_results(
        ...
        il_iec711_db,
        title_suffix="duct with Helmholtz resonator near 300 Hz",
    )
