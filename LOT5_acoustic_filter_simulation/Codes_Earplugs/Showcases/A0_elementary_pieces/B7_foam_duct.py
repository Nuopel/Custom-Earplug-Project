...
def plot_pressure_and_il_results(
    ...
    *,
    ...
    axes[0].semilogx(
        freqs_hz,
        20.0 * np.log10(np.maximum(np.abs(p_end_iec711_foam), np.finfo(float).tiny)),
        lw=2.0,
        label="Duct + foam, IEC711 load",
    )
...
if __name__ == "__main__":
    ...
    system_filter = inlet + foam_layer + outlet
    ...
    plot_pressure_and_il_results(
        ...
        il_iec711_db,
        title_suffix="duct with foam layer",
    )
