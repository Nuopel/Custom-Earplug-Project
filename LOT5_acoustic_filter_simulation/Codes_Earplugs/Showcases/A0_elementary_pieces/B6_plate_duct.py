...
def plot_pressure_and_il_results(
    ...
    *,
    ...
    axes[0].semilogx(
        freqs_hz,
        20.0 * np.log10(np.maximum(np.abs(p_end_iec711_plate), np.finfo(float).tiny)),
        lw=2.0,
        label="Duct + plate, IEC711 load",
    )
...
if __name__ == "__main__":
    ...
    freqs = np.logspace(np.log10(50.0), np.log10(10000.0), 300)
    ...
    plate = ExactFlexuralPlateSeriesImpedance(
        radius=r_duct,
    ...
    system_filter = inlet + halfduct + plate + halfduct + outlet
    ...
    plot_pressure_and_il_results(
        ...
        il_iec711_db,
        title_suffix="duct with flexural plate",
    )
